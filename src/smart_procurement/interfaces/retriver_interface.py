...
async def query_vector_db(
    text: str,
    embeeder: MixedbreadEmbeddingFunction,
    host: str | None = None,
    port: str | None = None,
) -> QueryResultList:
    if host and port:
...
if __name__ == "__main__":
    ...
    MEB_FUNC = MixedbreadEmbeddingFunction(embedder=EMBEDDER)
    query = "I want to buy keyboard and mouse for my Laptop."
    result = asyncio.run(query_vector_db(text=query, embeeder=MEB_FUNC))
